...
detectGD = 85
detectGDA = 160
detectGDB = 100 #di 95 kurang, drop di luar taman
detectGDC = 160
objectAvoid = 200
detectWall = 400
#altitude
...
def goto(coorX, coorZ, gd, string):
    print("vehicle goto : ", coorX, coorZ)
    sleep(0.5)
    toleransi = 20
    while True:
        ...
        if count > 8:
            ...
            if recv[0] == 'Y' and recv[1] == 'Y':     #python2
                ...
                if  pose > targetPose + 11:
                    mv.sendRemoteValue(None, None, None, yawRight, vehicle)
                    print("Right Yaw", pose, "desire :", targetPose, "dist:", distance, "x:", coorX, "alt:", current_altitude, string)
                elif pose < targetPose - 11:
                    mv.sendRemoteValue(None, None, None, yawLeft, vehicle)
                    print("Left Yaw",  pose, "desire :", targetPose, "dist:", distance, "x:", coorX, "alt:", current_altitude, string)
                elif x < coorX - toleransiX:
                    mv.sendRemoteValue(None, right, None, None, vehicle)
                    print("Right",  x, "desire :", coorX, "dist:", distance, "desire :", gd, "Yaw:", pose, "alt:", current_altitude, string)
                elif x > coorX + toleransiX:
                    mv.sendRemoteValue(None, left, None, None, vehicle)
                    print("left",  x, "desire :", coorX, "dist:", distance, "desire :", gd, "Yaw:", pose, "alt:", current_altitude, string)
                elif distance > gd + toleransi:
                    mv.sendRemoteValue(forward, None, None, 1525, vehicle)
                    print("forward",  distance, "desire :", gd, "Yaw:", pose, "x:", coorX, "alt:", current_altitude, string)
                elif distance <= gd - toleransi:
                    mv.sendRemoteValue(backwardFast, None, None, 1525, vehicle)
                    print("backward",  distance, "desire :", gd, "Yaw:", pose, "x:", coorX, "alt:", current_altitude, string)
                else :
                    mv.sendRemoteValue(None, None, None, None, vehicle)
                    print("reach distance : ", distance, "Desire: ", gd, "Yaw:", pose, "x:", coorX, "alt:", current_altitude, string)
                    ser.reset_input_buffer()
                    break
def Back(gd):
    print("vehicle goto Backward :", gd)
    #sleep(0.5)
    toleransi = 5
    while True:
        showMaps()
        ...
        if count > 8:
            ...
            if recv[0] == 'Y' and recv[1] == 'Y':     #python2
                ...
                if  pose > targetPose + 11:
                    mv.sendRemoteValue(None, None, None, yawRight, vehicle)
                    print("Right Yaw", pose, "desire :", targetPose, "dist:", distance)
                elif pose < targetPose - 11:
                    mv.sendRemoteValue(None, None, None, yawLeft, vehicle)
                    print("Left Yaw",  pose, "desire :", targetPose, "dist:", distance)

                if distance > gd + toleransi:
                    mv.sendRemoteValue(forward, None, None, None, vehicle)
                    print("forward",  distance, "desire :", gd, "Yaw:", pose)
                elif distance <= gd - toleransi:
                    mv.sendRemoteValue(backwardFast, None, None, None, vehicle)
                    print("backward",  distance, "desire :", gd, "Yaw:", pose)
                else :
                    mv.sendRemoteValue(None, None, 1500, None, vehicle)
                    print("reach distance : ", distance, "Desire: ", gd, "Yaw:", pose)
                    mv.sendRemoteValue(None, None, 1500, None, vehicle)
                    ser.reset_input_buffer()
                    print("Backward done....")
                    break
...
def goto_GD_A():
    ...
    sleep(0.1)
    set_pose(targetPose)
    goto(coorAx, coorAz, objectAvoid, "goto GD_A")
...
def goto_GD_B():
    ...
    sleep(0.1)
    set_pose(targetPose)
    goto(coorBx, coorBz, objectAvoid,  "goto GD_B")
...
def goto_GD_C():
    ...
    sleep(0.1)
    set_pose(targetPose)
    goto(coorCx, coorCz, objectAvoid,  "goto GD_C")
...
def goto_ELP():
    ...
    sleep(0.1)
    set_pose(targetPose)
    goto(coorObsx, coorObsz, objectAvoid, "Goto ELP")
def goto_area1():
    global succes
    print("Heading to ELP")
    sleep(0.1)
