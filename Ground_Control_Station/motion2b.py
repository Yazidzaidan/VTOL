...
def naik():
    #start = input("READY Naik")
    print("Motion Naik")
    up = False
    while up ==False:
        #start = input("READY Naik")
        print("Motion naik", " Rangefinder distance: %s" % vehicle.rangefinder.distance)
        if vehicle.rangefinder.distance < 1.2:
            for x in range(100):
                vehicle.channels.overrides['3'] = 1660
                print(x, "naik 1.2 meter")
                if vehicle.rangefinder.distance >= 1.2:
                    up = True
                    print("Reach Altitude Target, done")
                    vehicle.channels.overrides['3'] = 1520
                    vehicle.channels.overrides['3'] = None
                    print("stabilizing drone, UP = ", up)
                    time.sleep(1)
                    break
                if up == True:
                    vehicle.channels.overrides['3'] = 1520
...
def mauDrop():
    ...
    while True:
        ...
        if count > 8:
            ...
            if recv[0] == 0x59 and recv[1] == 0x59:     #python3
                distance2 = recv[2] + recv[3] * 256
                strength2 = recv[4] + recv[5] * 256
            ...
            if recv[0] == 'Y' and recv[1] == 'Y':     #python2
                ...
                lowS = int(recv[4].encode('hex'), 16)      
                highS = int(recv[5].encode('hex'), 16)
                distance2 = lowD + highD * 256
                strength2 = lowS + highS * 256
                print(distance2, strength2, "Detected wall = ", detectedWall)
            ...
            if distance2 <= 70 :
                ...
                break
        if detectedWall == True:
            print( "Detected wall = ", detectedWall, "Break Maju")
            break
...
def mundur():
    ...
    while True:
        ...
        if count > 8:
            ...
            if recv[0] == 0x59 and recv[1] == 0x59:     #python3
                distance3 = recv[2] + recv[3] * 256
                strength3 = recv[4] + recv[5] * 256
                print('(', distance3, ',', strength3, ')')
                ser.reset_input_buffer()
            if recv[0] == 'Y' and recv[1] == 'Y':     #python2
                lowD = int(recv[2].encode('hex'), 16)      
                highD = int(recv[3].encode('hex'), 16)
