...
def drawCoordinate(event, x, y, flags, frame):
    global a,b,c,d,e,f,g,h,count
    if event == cv2.EVENT_LBUTTONUP:
        nameGd = int(input('Where is the Buildings? '))
        if nameGd == 1  and count == 0:
            a = int(vals[0])
            b = int(vals[1])
            count = 1
        if nameGd == 2 and count == 1:
            c = int(vals[0])
            d = int(vals[1])
            count = 2
        if nameGd == 3 and count == 2:
            e = int(vals[0])
            f = int(vals[1])
            count = 3
        if nameGd == 4 and count == 3:
            g = int(vals[0])
            h = int(vals[1])
            count = 4
        data_Gd = {'Name':['Gd A','Gd B','Gd C','Obstacle'],
                    'CoordX':[a, c, e, g],
                    'CoordY':[b, d, f, h]}
        
        df_gd = pd.DataFrame(data_Gd) # tidak perlu kasih column name karena sudah ambil dari key yang ada di dalam dict
        if count == 4 :
            #df_gd.to_excel('coordinate.xlsx', index=False)
